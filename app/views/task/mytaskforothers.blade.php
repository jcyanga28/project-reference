...
<li><a href="{{URL::route('create.mytask.company')}}">COMPANY</a></li>
<li><a href="{{URL::route('create.mytask.project')}}">PROJECT</a></li>
<li class="active"><a href="{{URL::route('create.mytask.others')}}">OTHERS</a></li>
</ul>
		...
		{{ Form::open(array('method' => 'get','class' => 'form-inline')) }}
		 	...
		 	<div class="form-group">
		  	...
		  	@if(Input::get('status') < 2)
		  		<a href="{{ URL::route('newtask.forothers') }}" class="btn btn-primary btn_form_design"><i class="fa fa-plus"></i> New Task</a>
			@endif
		{{ Form::close() }}
	</div>
</div>
...
<div id="count_result" class="row">
	<div class="col-lg-12">
		...
		@if(count($mytask_forothers)>0)
		<span>{{ count($mytask_forothers) }}</span> record/s found &nbsp;
		@endif
	</div> 	
</div>
	...
	<div class="col-lg-12">
		<div class="table-responsive">
			<table id="tbl_form_design" class="table table-striped table-hover">
				...
				@else	
					@foreach($mytask_forothers as $row)				
					<tr>
						<td style="width: 20%;"><b>{{ strtoupper($row->task) }}</b></td>
						<td style="width: 25%;"><b>{{ date("m/d/Y", strtotime($row->date_start)) . ' TO ' . date("m/d/Y", strtotime($row->date_end)) }}</b></td>
						<td style="width: 55%;"><b>{{ strtoupper($row->remarks) }}</b></td>
						<td class="action">
							{{ HTML::linkRoute('task.forothers.details','Details', $row->id, array('class' => 'btn btn-warning btn-xs')) }}
						</td>	
					@if($row->status == 1)	
						<td class="action">
							{{ HTML::linkRoute('task.forothers.edit','Update', $row->id, array('class' => 'btn btn-info btn-xs')) }}
						</td>
					@endif
					@if($row->status == 1)
						<td class="action">

							{{ Form::open(array('method' => 'DELETE', 'route' => array('task.forothers.destroy', $row->id))) }}                       
