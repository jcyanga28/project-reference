...
<li class="active"><a href="{{URL::route('task.request.approver.forcompany')}}">COMPANY</a></li>
<li><a href="{{URL::route('task.request.approver.forproject')}}">PROJECT</a></li>
<li><a href="{{URL::route('task.request.approver.forothers')}}">OTHERS</a></li>
</ul>
		...
		<div class="table-responsive">
			<table id="tbl_form_design" class="table table-striped table-hover">
				...
				@else	
					...
					<tr>
						<td style="width: 20%;"><b>{{ strtoupper($row->company_name) }}</b></td>
					...
					@if($row->status < 3)	
						<td class="action">
							{{ HTML::linkRoute('task.approver.forcompany.details','Details', $row->id, array('class' => 'btn btn-warning btn-xs')) }}
						</td>
					@endif
					</tr>
					@endforeach
