...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-6 col-md-12">
		...
		<tr>
			<td id="tbl_details_td">PROJECT</td><td>{{ strtoupper($details->project_name) }}</td>
		</tr>
		<tr>
			...
			<table style="font-size: 12px; width: 50%;" class="table table-hover">
				...
				@foreach($getattached as $filesrow)
					<tr>
						...
						<td style="font-size: 12px;"><a href="/project/download/{{ $filesrow->file_img }}">DOWNLOAD</a></td>
					</tr>
				@endforeach
			</table>
		@else
				 	 	...
				 	 	@if(isset($row->files))
					 	 	@foreach($row->files as $row_file)
					 	 		<tr>		
					 	 			...
					 	 			<td style="font-size: 12px;"><a href="/download-request/project-thread/{{ $row_file->filename }}">DOWNLOAD</a></td>
					 	 		</tr>	
					 	 	@endforeach
				 	 	@endif
				 	 	@if(isset($row->file))
					 	 	@foreach($row->file as $row_files)
					 	 		<tr>		
					 	 			...
					 	 			<td style="font-size: 12px;"><a href="/download-request/project-thread/{{ $row_files->filename }}">DOWNLOAD</a></td>
					 	 		</tr>	
					 	 	@endforeach
				 	 	@endif
				 	</table>
	...
	@if($details->approved_request == 1)
		{{ Form::open(array('route' => 'task.forproject.approver.tagremarks','class' => 'bs-component', 'files' => true)) }} 				
			<diV id="closes_chkbox">{{ Form::checkbox('closed') }} &nbsp;APPROVED/CLOSED REQUEST.<em style="color:#F00;font-size: 13px;"> (Leave this box if just commenting on the request)</em></div>
