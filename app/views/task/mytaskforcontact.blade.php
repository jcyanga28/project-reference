...
<li class="active"><a href="{{URL::route('create.mytask')}}">CONTACT</a></li>
		...
		{{ Form::open(array('method' => 'get','class' => 'form-inline')) }}
		 	...
		 	<div class="form-group">
		  	...
		  	@if(Input::get('status') < 2)
		  		<a href="{{ URL::route('newtask.forcontact') }}" class="btn btn-primary btn_form_design"><i class="fa fa-plus"></i> New Task</a>
			@endif
		{{ Form::close() }}
	</div>
</div>
...
<div id="count_result" class="row">
	<div class="col-lg-12">
		...
		@if(count($mytask_forcontact)>0)
		<span>{{ count($mytask_forcontact) }}</span> record/s found &nbsp;
		@endif
	</div> 	
</div>
	...
	<div class="col-lg-12">
		<div class="table-responsive">
			<table id="tbl_form_design" class="table table-striped table-hover">
				...
				@else	
					@foreach($mytask_forcontact as $row)				
					<tr>
						<td style="width: 25%;"><b>{{ strtoupper($row->fullname) }}</b></td>
						<td style="width: 25%;"><b>{{ strtoupper($row->task) }}</b></td>
						<td style="width: 50%;"><b>{{ strtoupper($row->remarks) }}</b></td>
						<td class="action">
							{{ HTML::linkRoute('task.forcontact.details','Details', $row->id, array('class' => 'btn btn-warning btn-xs')) }}
						</td>
					@if($row->status == 1)	
						<td class="action">
							{{ HTML::linkRoute('task.forcontact.edit','Update', $row->id, array('class' => 'btn btn-info btn-xs')) }}
						</td>
					@endif
					@if($row->status == 1)
						<td class="action">

							{{ Form::open(array('method' => 'DELETE', 'route' => array('task.forcontact.destroy', $row->id))) }}                       
