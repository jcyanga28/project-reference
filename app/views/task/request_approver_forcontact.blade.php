...
<li class="active"><a href="{{URL::route('task.request.approver.forcontact')}}">CONTACT</a></li>
		...
		<div class="table-responsive">
			<table id="tbl_form_design" class="table table-striped table-hover">
				...
				@else	
					...
					<tr>
						<td style="width: 20%;"><b>{{ strtoupper($row->fullname) }}</b></td>
					...
					@if($row->status < 3)	
						<td class="action">
							{{ HTML::linkRoute('task.approver.forcontact.details','Details', $row->id, array('class' => 'btn btn-warning btn-xs')) }}
						</td>
					@endif
					</tr>
					@endforeach
