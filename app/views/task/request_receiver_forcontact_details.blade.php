...
<div class="row">
	...
	<table>
		<tr>
			<td>
				{{ Form::label('remarks_desc', '*Remarks', array('class' => 'control-label')) }} <em style="color:#F00;font-size: 11px;">(Note : attach file or image if you send this request to Approver.)</em><br/>
	...
	@if($details->approved_request < 1)	
	 	 {{ Form::open(array('route' => 'task.forcontact.approve','class' => 'bs-component', 'files' => true)) }} 				
	...
	@if($details->approved_request < 1)	
	 	 {{ Form::open(array('method' => 'PUT', 'route' => array('task.forcontact.decline', $details->id))) }} 
				 	 	...
				 	 	@if(isset($row->files))
					 	 	@foreach($row->files as $row_file)
					 	 		<tr>		
					 	 			...
					 	 			<td style="font-size: 12px;"><a href="/download-request/contact-thread/{{ $row_file->filename }}">DOWNLOAD</a></td>
					 	 		</tr>	
					 	 	@endforeach
				 	 	@endif
				 	 	@if(isset($row->file))
					 	 	@foreach($row->file as $row_files)
					 	 		<tr>		
					 	 			...
					 	 			<td style="font-size: 12px;"><a href="/download-request/contact-thread/{{ $row_files->filename }}">DOWNLOAD</a></td>
					 	 		</tr>	
					 	 	@endforeach
				 	 	@endif
				 	</table>
	...
	@if($details->approved_request == 1)
		{{ Form::open(array('route' => 'task.forcontact.approve.tagremarks','class' => 'bs-component', 'files' => true)) }} 				
