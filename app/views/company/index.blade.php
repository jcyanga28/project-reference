		...
		{{ Form::open(array('method' => 'get','class' => 'form-inline')) }}
		 	...
		 	<div class="form-group">
		  	...
		  	@if(Input::get('status') < 2)
		  		<a href="{{ URL::route('company.create') }}" class="btn btn-primary btn_form_design"><i class="fa fa-plus"></i> New Company</a>
			@endif
		{{ Form::close() }}
	</div>
</div>
			...
			<table id="tbl_form_design" class="table table-striped table-hover">
				...
				@else	
					...
					<tr>
						...
						<td class="action">
							{{ HTML::linkRoute('company.dt','Details', $row->id, array('class' => 'btn btn-warning btn-xs')) }}
						</td>
					@if($row->status < 3)	
						<td class="action">
							{{ HTML::linkRoute('company.edit','Update', $row->id, array('class' => 'btn btn-info btn-xs')) }}
						</td>
					@endif
					@if($row->status < 2)
						<td class="action">
							{{ Form::open(array('method' => 'DELETE', 'route' => array('company.destroy', $row->id))) }}                       
