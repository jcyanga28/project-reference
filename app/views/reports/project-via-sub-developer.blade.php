...
<div class="row">
	...
	{{ Form::open(array('method' => 'get','class' => 'bs-component')) }}
		...
		<div class="form-group @if ($errors->has('project_developer')) has-error @endif">
			@if(isset($project_developer))
				<?php
					$dev = $project_developer->fullname;
					$dev_id = $project_developer->id;
				?>
			@else
				<?php
					$dev = 'CHOOSE DEVELOPER HERE';
					$dev_id = '0';
				?>
			@endif
			{{ Form::label('project_developer_desc', 'PROJECT DEVELOPER') }}
			{{ Form::select('project_developer', array($dev_id => $dev) + $developer, null, array('class' => 'form-control project_developer_report', 'id' => 'developers')) }}
			@if ($errors->has('project_developer')) <p class="help-block">{{ $errors->first('project_developer') }}</p> @endif
		</div>
...
<div class="row">
	...
	@if(isset($project_via_sub_developer))
		@if(count($project_via_sub_developer)>0)
		{{ Form::open(array('route' => 'generate.projectsviasubdevelopers.report', 'method' => 'POST','class' => 'bs-component')) }}
				<input type="hidden" name="hid_agree_val" @if(isset($_GET['agree_value'])) value="{{ Input::get('agree_value') }}" @endif id="agree_value" />
				<input type="hidden" name="hid_proj_dev" @if(isset($_GET['project_developer'])) value="{{ Input::get('project_developer') }}" @endif id="project_developer" />
	...
	<table id="tbl_form_design" class="table table-striped">
		...
		<tbody>
			@if(isset($project_via_sub_developer))
				@if(count($project_via_sub_developer) > 0)
					@foreach($project_via_sub_developer as $row)	
...
<script type="text/javascript">
	...
	var domain ="http://"+document.domain;
		...
		$("#developers").chosen({allow_single_deselect: true});
