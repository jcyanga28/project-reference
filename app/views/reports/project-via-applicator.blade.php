...
<div class="row">
	...
	@if(isset($project_via_applicator))
		@if(count($project_via_applicator)>0)
		{{ Form::open(array('route' => 'generate.projectsviaapplicators.report', 'method' => 'POST','class' => 'bs-component')) }}
	...
	<table id="tbl_form_design" class="table table-striped">
		...
		<tbody>
			@if(isset($project_via_applicator))
				@if(count($project_via_applicator) > 0)
					@foreach($project_via_applicator as $row)	
