...
<div class="row">
	...
	{{ Form::open(array('method' => 'get','class' => 'bs-component')) }}
		...
		<br/>	
		<div class="form-group @if ($errors->has('architect')) has-error @endif">
			@if(isset($architect))
				<?php
					$archs = $architect->fullname;
					$archs_id = $architect->id;
				?>
			@else
				<?php
				...
				?>
			@endif
			{{ Form::label('architect_desc', 'ARCHITECT') }}<br/>
			{{ Form::select('architect', array($archs_id => $archs) + $arch, null, array('class' => 'form-control architect_report', 'id' => 'architects')) }}
...
<div class="row">
	...
	@if(isset($project_via_architect))
		@if(count($project_via_architect)>0)
		{{ Form::open(array('route' => 'generate.projectsviaarchitects.report', 'method' => 'POST','class' => 'bs-component')) }}
				<input type="hidden" name="hid_agree_val" @if(isset($_GET['agree_value'])) value="{{ Input::get('agree_value') }}" @endif id="agree_value" />
				<input type="hidden" name="hid_arch" @if(isset($_GET['architect'])) value="{{ Input::get('architect') }}" @endif id="architect" />
	...
	<table id="tbl_form_design" class="table table-striped">
		...
		<tbody>
			@if(isset($project_via_architect))
				@if(count($project_via_architect) > 0)
					@foreach($project_via_architect as $row)	
...
<script type="text/javascript">
	...
	var domain ="http://"+document.domain;
		...
		if(agree == 1)
		{
			$("#architects").prop('disabled', true);
			// location.reload();
		}

		$("#architects").chosen({allow_single_deselect: true});
		...
		$(this).change(function(){
			...
			if($('.search_all').is(':checked')) 
			{
				...
				$("#architects").prop('disabled', false);
				location.reload();
				$('#agree_value').val('0');
			}
