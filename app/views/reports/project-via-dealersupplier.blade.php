...
<div class="row">
	...
	{{ Form::open(array('method' => 'get','class' => 'bs-component')) }}
		...
		<div class="form-group @if ($errors->has('dealer_supplier')) has-error @endif">
			...
			@else
				<?php
					$dealsupps = 'CHOOSE DEALER/SUPPLIER HERE';
					$dealsupps_id = '0';
				?>
			@endif
...
<div class="row">
	...
	@if(isset($project_via_dealsupp))
		@if(count($project_via_dealsupp)>0)
		{{ Form::open(array('route' => 'generate.projectsviadealersuppliers.report', 'method' => 'POST','class' => 'bs-component')) }}
	...
	<table id="tbl_form_design" class="table table-striped">
		<thead>
			<tr>
				<td><b>DEALER/ SUPPLIER NAME</b></td>
		...
		<tbody>
			@if(isset($project_via_dealsupp))
				@if(count($project_via_dealsupp) > 0)
					@foreach($project_via_dealsupp as $row)	
